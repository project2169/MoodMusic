...
class Model:
    ...
    def fit(self, epochs):
        ...
        self.model.save("best_age_model.h5")
    
    def evaluate(self):
        self.model.evaluate(VAL_DS)
    
    def pred(self):
        img_pred = image.load_img('/age/validation/21-30/308.png', target_size=(48, 48))
...
if __name__ == "__main__":
    model = Model(Adam(learning_rate = 0.01), CategoricalCrossentropy()) # Change optimizer and loss function here
    model.fit(epochs = 100) # Change number of epochs here
    model.evaluate()
    model.pred()